...
module.exports = {
    ...
    //    creep.say ("up: " + + creep.ticksToLive);
       ...
       util.TimeToDie2(creep,"W15S17",(workBodyParts * 50/5) + 5,0); // die before getting more energy
      
      
       // console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] ' + creep.name + ' total workBodyParts is ' + workBodyParts +'</>');
       ...
       var workStatus = workCheck(creep);
        ...
        // ********************************************************************************//;
               ...
               else if (creep.room.name == "E44S3" || creep.room.name == "E43S3"  || creep.room.name == "E45S2" || creep.room.name == "W14S18") {
...
function myGetActiveBodyparts (creep,type, count) 
{
    let count = 0;
    for (let i = creep.body.length; i-- > 0;) {
